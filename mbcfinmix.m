function [pies,mus,vars]=mbcfinmix(data,muin,varin,piesin,model)
...
oldlike = likelihood(data,mu,var_mat,mix_cof);
...
while num_it <= max_it & ~isequal(oldlike,newlike)
    ...
    muf = muupm(data,posterior,mix_coff);
    varf = varupm(data,posterior,mix_coff,muf,model);
	
	% reset parameters
    newlike = likelihood(data,muf,varf,mix_coff);
    num_it = num_it+1;
    mix_cof = mix_coff;
    mu = muf;
    var_mat = varf;
